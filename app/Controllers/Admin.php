...
class Admin extends BaseController
{
    ...
    public function tipeKamarSave()
    {
        ...
        if ($foto && $foto->isValid() && !$foto->hasMoved()) {
            $newName = $foto->getRandomName();
            $foto->move(FCPATH . 'uploads/akomodasi', $newName);
            $data['foto'] = $newName;
        }
    ...
    public function userTambah()
    {
        ...
        // Cek role tidak boleh pelanggan
        if ($this->request->getPost('role') === 'pelanggan') {
            return redirect()->back()->with('error', 'Tidak dapat menambahkan user dengan role Pelanggan!');
        }
   ...
   public function laporanGenerate()
{
    ...
    // Ambil data pemesanan berdasarkan rentang tanggal
...
{
    $booking = $this->pemesananModel->getDetailPemesananFull($id);
    ...
    return redirect()->to('/admin/booking')->with('success', 'Booking ' . esc($booking['nama_tamu']) . ' berhasil dihapus!');
}
